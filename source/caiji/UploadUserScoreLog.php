...
$db = new db();
$sql = "select id,uid,change_points,thetime,remark from user_score_changelog where status=0 and (remark LIKE '手动投注后%' OR remark LIKE '自动投注后%' OR remark='开奖后')";
...
foreach($logres as $log){
    ...
    if (!empty($username)) {
	    ...
	    $url = 'http://192.168.1.36:13201/lottery/fundflow';
