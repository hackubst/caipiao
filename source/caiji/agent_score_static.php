...
$score_sql = 
    ...
    WHERE (a.remark LIKE '手动投注后%' OR a.remark LIKE '自动投注后%') 
        AND DATE_FORMAT(a.thetime,'Y-m-d')=DATE_SUB(NOW(),INTERVAL 1 DAY) 
        AND b.uid IS NULL AND b.uid>0
...
foreach($logres as $item){
    ...
    // 第一步记录业绩分成结果
    $into_sql = "INSERT INTO agent_score_static(agentid,`level`,agentpid,win_points,don_points,sys_points,tme_points,one_points,two_points,thr_points,thedate)
     VALUES({$uid},{$level},{$pid},{$win_points},{$don_points},{$sys_points},{$tme_points},{$one_points},{$tow_points},{$thr_points},NOW())";
 ...
 */
function up_agent_score($uid, $scores, $level = 0)
{
    if ($level<=3) 
    {
        //更新业绩余额
        $upd_sql = "update agent set score=score+{$scores[$level]} where uid={$uid}";
        $upd_ret = $db->execute($upd_sql);
        if($upd_ret===FALSE) {
            $db->execute("rollback");
            continue;
        } else {
            // 记录业绩记录
            $into_sql = "INSERT INTO agent_score_log(agentid,`level`,ext_points,thedate) VALUES({$uid},{$level},{$scores[$level]},NOW())";
            $into_ret = $db->execute($into_sql);
            if($into_ret===FALSE) {
                $db->execute("rollback");
                continue;
        ...
        $sel_pid = $db->getOne($sel_sql);
        if (!empty($sel_pid) && intval($sel_pid)>0) {
            //递归 更新上级代理业绩
            up_agent_score($sel_pid, $scores, ($level+1));
        }
    }
}
